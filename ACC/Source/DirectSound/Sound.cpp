...
//		Soundデバイス作成
//==================================================================================================
HRESULT Sound::CreateDevice( HWND hWnd )
{
	// エフェクトの使用前にCOMを初期化
...
//		データ読込
//==================================================================================================
HRESULT Sound::LoadData( LPSTR szFileName )
{
...
//		常に最初から再生する
//==================================================================================================
HRESULT Sound::Play(const GUID* pGuid)
{
...
//		ループ再生
//==================================================================================================
void Sound::PlayLoop( const GUID *pGuid )
{
...
//		常に最初からエフェクト付きで再生する
//==================================================================================================
HRESULT Sound::PlayEx( I3DL2_ENV_PRESET preset )
{
	if( FAILED( m_pDSBuffer->Stop() ) == S_OK ){
		// 巻き戻し
		m_pDSBuffer->SetCurrentPosition( 0 );
...
//		エフェクト付きでループ再生
//==================================================================================================
void Sound::playLoopEx( I3DL2_ENV_PRESET preset )
{
	m_pDSBuffer->Play(0, 0, DSBPLAY_LOOPING);
	GetPreset(preset);
}
...
//		周波数を設定する
//==================================================================================================
void Sound::SetFrequency( DWORD freq )
{
...
//		音量設定のための反転処理
//==================================================================================================
LONG Sound::Mapvol(LONG volume)
{
	volume = max(0L, min(volume, 10000L));
...
//		エフェクトパラメータの取得
//=============================================================================
DWORD Sound::GetPreset( I3DL2_ENV_PRESET presetNo )
{
...
//		再生関数
//----------------------------------------------------------------------------
HRESULT Sound::PlayBase(const GUID* pGuid, DWORD preset)
{
	// 現在のボリューム退避
	LONG lVolume;
	m_pDSBuffer->GetVolume(&lVolume);
...
//		エフェクトパラメータを設定・適用する
//--------------------------------------------------------------------------------------------------
HRESULT Sound::SetEffectParam(LPDIRECTSOUNDBUFFER8 pDSBuffer8, DWORD dwPreset)
{
...
//-----------------------------------------------------------------------------
LPWSTR Sound::ConvertLPSTRToLPWSTR(LPSTR lpstr) 
{
...
//-----------------------------------------------------------------------------
LONG Sound::RoundingPan( LONG pan )
{
...
//-----------------------------------------------------------------------------
DWORD Sound::RoundingFrequency( DWORD freq )
{
	if( freq != DSBFREQUENCY_ORIGINAL )
	{
		freq = std::clamp( freq, (DWORD)DSBFREQUENCY_MIN, (DWORD)DSBFREQUENCY_MAX );
	}
	return freq;
}
...
//-----------------------------------------------------------------------------
LONG Sound::RoundingVolume( LONG volume )
{
