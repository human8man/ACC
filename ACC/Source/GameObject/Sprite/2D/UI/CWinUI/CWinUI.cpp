...
void CWinUI::Update()
{
	...
	for (size_t i = 0; i < m_pUIs.size(); ++i) {
		// 背景の処理はいらないので早期に切る.
		if (i == WinSprite::FullScreen) { continue; }
		...
		if (i == WinSprite::StartButton && m_pUIs[i]->GetPatternNo().x) {
			if (Mouse->IsLAction()) {
				// ゲームを開始する.
...
//		初期化.
//------------------------------------------------------------------------------------------------
bool CWinUI::PointInSquare( POINT ppos, D3DXVECTOR2 spos, D3DXVECTOR2 sposs)
{
	if (spos.x < ppos.x
	&&  spos.y < ppos.y
